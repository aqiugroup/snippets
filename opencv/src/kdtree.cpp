...
void TestKdtree(int seedNum)
{
    std::cout << " seedNum " << seedNum << std::endl;
    // generate space
    const int width = 500;
    const int height = 500;
    cv::Mat img = cv::Mat::zeros(cv::Size(width, height), CV_8UC3);
    // generate points
    const int npoints = seedNum;
    ...
    cv::circle(img, cv::Point2d(query), 1, cv::Scalar(0, 0, 255), 2);
    // nearest neigbor search
    const cv::Mat I0 = img.clone();
    const int idx = kdtree.nnSearch(query);
    cv::circle(I0, cv::Point2d(points[idx]), 1, cv::Scalar(255, 255, 0), 3);
