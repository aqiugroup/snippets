// ref: https://github.com/gishi523/kd-tree
//
#pragma once
#include "MyUtils.h"
    ...
    public:
        ...
        */
        void build(const std::vector<PointT> &points)
        {
            ...
            std::iota(std::begin(indices), std::end(indices), 0); // 记录index，数据存在points_中
        ...
        */
        ...
        {
            KnnQueue queue(k); // 为啥不直接用一个PriorityQueue，大顶堆 ？
    ...
    private:
        ...
        */
        Node *buildRecursive(int *indices, int npoints, int depth)
        {
            ...
            }); // 随机取中间的数字，将数组分为两拨（一边小于mid对应的值，一边大于mid的值）
            Node *node = new Node();
        ...
        */
        void nnSearchRecursive(const PointT &query, const Node *node, int *guess, double *minDist) const
        {
            ...
            const int axis = node->axis;
            const int dir = query[axis] < train[axis] ? 0 : 1;  // 待查询的数字比当前节点要小，搜索左子树，否则所搜右子树
            nnSearchRecursive(query, node->next[dir], guess, minDist);
            const double diff = fabs(query[axis] - train[axis]); // 待查询的数字与当前分割线的距离比minDist还小，说明分割线另外一侧有可能还有更小的值满足要求，所以也需要搜索
            if (diff < *minDist) {
                nnSearchRecursive(query, node->next[!dir], guess, minDist);
            }
        }
