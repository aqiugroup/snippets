...
int main(int argc, char **argv)
{
    ...
    std::cout << "LU分解所用时间和解为: " << 1000 * (clock() - tim_stt) / (double)CLOCKS_PER_SEC
              << "MS" << std::endl << x.transpose() << std::endl;
#else
    ...
    std::cout << "Cholesky分解方法所用时间和解为: " << (1000 * clock() - tim_stt) / (double)CLOCKS_PER_SEC
              << "MS" << std::endl << x.transpose() << std::endl;
#else
    ...
    std::cout << "Jacobi迭代法所用时间和解为：" << 1000 * (clock() - tim_stt) / (double)CLOCKS_PER_SEC
              << "MS" << std::endl << x.transpose() << std::endl;
#else
    std::cout << "LU分解法不能解!(提示:直接法中方程组的个数必须与变量个数相同，需要设置MATRIX_SIZE == MATRIX_SIZE_)" << std::endl;
#endif
...
Mat_B Jacobi(Mat_A  &A, Mat_B  &b, int &iteration_num, double &accuracy )
{
    ...
    for(int index = 0; index < MATRIX_SIZE; index++) {
        maxev = ( maxev > abs(EV(index)) ) ? maxev : (abs(EV(index)));
